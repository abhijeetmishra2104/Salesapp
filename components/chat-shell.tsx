...
import { Send, Mic, User, Bot, Loader2, ShoppingCart, ChevronRight } from "lucide-react"
import { getRecommendations, fetchInventory, scheduleFulfillment, getLoyaltyInfo } from "@/lib/api"
...
export function ChatShell() {
  ...
  const handleSend = async () => {
    ...
    if (lowerInput.includes("wedding") || lowerInput.includes("shoe") || lowerInput.includes("recommend")) {
      ...
      await invokeAgent(
        ...
        (data: unknown) => {
          const typedData = data as {
            recommendations: Array<{ name: string; sku: string; price: number; salePrice?: number }>
          }
          const recs = typedData.recommendations
          if (recs && recs.length > 0) {
            const recList = recs
              .map(
                (r: { name: string; sku: string; price: number; salePrice?: number }) =>
      ...
      await invokeAgent(
        "inventory",
        () => fetchInventory(state.cart[0]?.sku || "SKU-HEEL-001", state.mockSettings.inventoryMode),
        (data: unknown) => {
          const typedData = data as { inventory: Record<string, number>; stores: Array<{ name: string }> }
          ...
          if (online > 5) {
            ...
            addAgentMessage(
              ...
              "inventory",
              ["Reserve at Store #2", "Notify when available", "Show similar items"],
            )
          }
        },
      )
      ...
      await invokeAgent(
        ...
        (data: unknown) => {
          ...
          if (slots && slots.length > 0) {
            addAgentMessage(
              `I can reserve this for pickup at ${typedData.store?.name || "our store"}. Available slots:\n\n• ${slots[0].date}: ${slots[0].times.slice(0, 3).join(", ")}\n\nWhich time works best for you?`,
              "fulfillment",
      ...
      await invokeAgent(
        ...
        (data: unknown) => {
          ...
          addAgentMessage(
            `As a ${typedData.tier} member, you get ${typedData.tierPercentage}% off! You have ${typedData.currentPoints} points (worth $${(typedData.currentPoints * 0.01).toFixed(2)}) and $${typedData.giftCardBalance} gift card balance. Would you like to apply these to your order?`,
            "loyalty",
  ...
  return (
    <div className={cn("flex flex-col h-[calc(100vh-8rem)] transition-all", channelStyles[state.currentChannel])}>
      {/* Header */}
      <div className="flex flex-col gap-4 p-4 border-b border-border">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-semibold">OmniSales AI Assistant</h2>
            <p className="text-sm text-muted-foreground">
              {state.customer ? `Hello, ${state.customer.name}` : "Your personal shopping assistant"}
            </p>
          </div>
          {state.cart.length > 0 && (
            <Link href="/checkout">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {state.messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
            ...
            <p className="text-lg font-medium">Welcome to OmniSales AI</p>
            <p className="text-sm">Start a conversation or try a quick action below</p>
            <div className="flex flex-wrap gap-2 mt-4 justify-center">
              {["Shopping for wedding shoes", "Show me sale items", "Check my loyalty points"].map((suggestion) => (
                <Button key={suggestion} variant="outline" size="sm" onClick={() => handleQuickReply(suggestion)}>
                  {suggestion}
                </Button>
              ))}
            </div>
